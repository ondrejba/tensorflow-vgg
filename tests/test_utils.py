...
class TestUtils(unittest.TestCase):
    ...
    def test_find_and_replace_max(self):

        vals = [30, -20, 5, 7]
        val = 6

        self.assertEqual(utils.find_and_replace_max(val, vals), 1)
        self.assertEqual(vals, [30, 6, 5, 7])

        self.assertEqual(utils.find_and_replace_max(val, vals), 2)
        self.assertEqual(vals, [30, 6, 6, 7])

        self.assertEqual(utils.find_and_replace_max(val, vals), None)
        self.assertEqual(vals, [30, 6, 6, 7])